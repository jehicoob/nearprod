...
interface ArchivedDatabase extends Database {
    archivedAt: string;
    lifecycle: 'archived';
}
interface Binding {
...
interface Infra {
    ...
    archivedDatabases?: ArchivedDatabase[];
    bindings: Binding[];
    connected: boolean;
...
function Review({ value, onConfirm, onClose, busy }: {
    ...
    const lifecycleAction = String(value.lifecycleAction || '');
    const lifecycleInstance = value.instance as { id: string; name: string; requestedImage: string; location: string } | undefined;
    const lifecycleDatabases = value.databases as { id: string; name: string; username: string; state: string }[] | undefined;
    const lifecycleDatabase = value.database as { id: string; name: string; username: string; state: string } | undefined;
    const lifecycleDatabaseInstance = value.instance as { id: string; name: string; engine: string; location: string } | undefined;
    const preserved = value.preserved as string[] | undefined;
    ...
    {lifecycleInstance && lifecycleAction === 'archive-instance' && <><Alert><strong>Se retirará solo del catálogo activo.</strong> Los recursos Docker, credenciales, carpetas, volúmenes e imágenes se conservarán.</Alert><section className="panel"><h3>Archivar {lifecycleInstance.name}</h3><p className="mono">{lifecycleInstance.id} · {lifecycleInstance.requestedImage}</p><p><strong>Datos:</strong> {lifecycleInstance.location}</p><p><strong>Bases conservadas:</strong> {lifecycleDatabases?.length || 0}</p>{preserved && <ul>{preserved.map(item => <li key={item}>{item}</li>)}</ul>}</section></>}
    {lifecycleInstance && lifecycleAction === 'restore-instance' && <><Alert><strong>Volverá al catálogo activo sin iniciar el runtime.</strong> No se modificarán contenedores ni datos.</Alert><section className="panel"><h3>Restaurar {lifecycleInstance.name}</h3><p className="mono">{lifecycleInstance.id} · {lifecycleInstance.requestedImage}</p><p><strong>Datos:</strong> {lifecycleInstance.location}</p><p><strong>Bases restauradas:</strong> {lifecycleDatabases?.length || 0}</p></section></>}
    {lifecycleDatabase && lifecycleAction === 'archive-database' && <><Alert><strong>Archivo reversible.</strong> La base física, su usuario y credencial se conservarán.</Alert><section className="panel"><h3>Archivar {lifecycleDatabase.name}</h3><p className="mono">{lifecycleDatabase.id} · {lifecycleDatabase.username}</p><p>Instancia: {lifecycleDatabaseInstance?.name} · Datos: {lifecycleDatabaseInstance?.location}</p></section></>}
    {lifecycleDatabase && lifecycleAction === 'restore-database' && <><Alert>La metadata volverá al catálogo activo sin iniciar el motor ni modificar datos.</Alert><section className="panel"><h3>Restaurar {lifecycleDatabase.name}</h3><p className="mono">{lifecycleDatabase.id} · {lifecycleDatabase.username}</p></section></>}
    {lifecycleDatabase && lifecycleAction === 'purge-database' && <><Alert error><strong>Operación irreversible.</strong> Se eliminarán físicamente la base y su cuenta limitada. No se tocarán bases hermanas.</Alert><section className="panel"><h3>Purgar {lifecycleDatabase.name}</h3><p className="mono">{lifecycleDatabase.id} · {lifecycleDatabase.username}</p><p>Modalidad: {value.mode === 'backup-purge' ? 'Crear backup verificado y después purgar' : 'Purgar sin backup'}</p><p>Instancia: {lifecycleDatabaseInstance?.name} · {lifecycleDatabaseInstance?.engine}</p></section></>}
    ...
    <div className="modal-actions"><button onClick={onClose}>Volver</button><button className={['archive-instance','archive-database','purge-database'].includes(lifecycleAction) ? 'danger' : 'primary'} disabled={busy} onClick={onConfirm}>{lifecycleAction === 'archive-instance' ? 'Archivar del catálogo' : lifecycleAction === 'restore-instance' ? 'Restaurar en el catálogo' : lifecycleAction === 'archive-database' ? 'Archivar base' : lifecycleAction === 'restore-database' ? 'Restaurar base' : lifecycleAction === 'purge-database' ? value.mode === 'backup-purge' ? 'Crear backup y purgar' : 'Purgar sin backup' : 'Confirmar y aplicar'}</button></div>
  </Modal>;
}
export function InfrastructureView({ catalog, status, notify }: Props) {
    ...
    const [create, setCreate] = useState(false), [dbFor, setDbFor] = useState<Instance | null>(null), [connection, setConnection] = useState<Database | null>(null), [link, setLink] = useState<Database | null>(null), [dbLifecycle, setDbLifecycle] = useState<{db: Database; engine: Engine} | null>(null), [logs, setLogs] = useState<Instance | null>(null), [backup, setBackup] = useState<{
        db: Database;
        restore: boolean;
    } | null>(null);
    ...
    {!!data?.archivedInstances?.length && <section className="archived-instances"><div className="section-heading compact"><div><span className="eyebrow">FUERA DEL CATÁLOGO ACTIVO</span><h2>Instancias archivadas</h2><p>Sus identidades, recursos y rutas siguen reservados. Restaurar no inicia el runtime.</p></div></div>{data.archivedInstances?.map(r => <section className="panel infra-instance" key={r.uid}><div className="panel-heading"><Icon name="cube"/><div><h3>{r.name}</h3><span className="mono muted">{r.id} · {r.requestedImage}</span></div><Badge value="archived"/></div><div className="infra-facts"><div><span>Persistencia conservada</span><code>{r.location}</code></div><div><span>Bases conservadas</span><strong>{r.databaseCount}</strong></div><div><span>Archivada</span><time>{new Date(r.archivedAt).toLocaleString()}</time></div></div><div className="button-row"><button className="primary" disabled={!canMutate} onClick={() => { void api<Preview>('/infra/restore-instance-preview', { instance: r.id }).then(value => setReview({ value, body: { action: 'restore-instance', instance: r.id } })).catch(e => setError(message(e))); }}>Restaurar en el catálogo</button></div></section>)}</section>}
    {create && data &&
      <CreateInstance data={data} onClose={() => setCreate(false)} onReview={(body, value) => { setCreate(false); setReview({ body: { ...body, action: 'create' }, value }); }}/>
    }
    {dbFor &&
      <CreateDatabase instance={dbFor} onClose={() => setDbFor(null)} onSubmit={body => { setDbFor(null); void action(body); }}/>
    }
    {connection &&
      <ConnectionDialog database={connection} onClose={() => setConnection(null)}/>
    }
    {link &&
      <BindingDialog database={link} engine={data?.instances.find(r => r.uid === link.instanceUid)?.engine || 'postgres'} stacks={catalog.stacks} onClose={() => setLink(null)} onReview={(body, value) => { setLink(null); setReview({ value, body: { ...body, action: 'bind' } }); }}/>
    }
    {dbLifecycle && <DatabaseLifecycleDialog {...dbLifecycle} onClose={() => setDbLifecycle(null)} onReview={(body,value) => {setDbLifecycle(null);setReview({body,value});}}/>}
...
function DatabaseLifecycleDialog({db,engine,onClose,onReview}: {db: Database; engine: Engine; onClose: () => void; onReview: (body: Data,value: Preview) => void}) {
    const [mode,setMode] = useState<'archive'|'backup-purge'|'purge'>('archive'), [directory,setDirectory] = useState(''), [typedId,setTypedId] = useState(''), [acknowledge,setAcknowledge] = useState(false), [busy,setBusy] = useState(false), [error,setError] = useState('');
    const destructiveUnavailable = engine === 'redis';
    async function submit(e: FormEvent) {e.preventDefault();setBusy(true);setError('');try {const body: Data = mode === 'archive' ? {action:'archive-database',database:db.id} : {action:'purge-database',database:db.id,mode,directory:directory || undefined,typedId,acknowledgeDataLoss:acknowledge};const endpoint = mode === 'archive' ? '/infra/archive-database-preview' : '/infra/purge-database-preview';onReview(body,await api<Preview>(endpoint,body));} catch(e) {setError(message(e));} finally {setBusy(false);}}
    const ready = mode !== 'purge' || acknowledge && typedId === db.id;
    return <Modal title="Gestionar ciclo de vida de base" subtitle={`${db.name} · ${db.id}`} onClose={onClose} wide><form onSubmit={e => void submit(e)}><fieldset className="lifecycle-options"><legend>Acción de ciclo de vida</legend><label><input type="radio" name="database-lifecycle" checked={mode === 'archive'} onChange={() => setMode('archive')}/><span><strong>Archivar base (reversible)</strong><small>Recomendado. Conserva la base física, usuario y credencial; retira solo metadata activa.</small></span></label><label><input type="radio" name="database-lifecycle" checked={mode === 'backup-purge'} disabled={destructiveUnavailable} onChange={() => setMode('backup-purge')}/><span><strong>Crear backup y purgar</strong><small>Exporta y verifica un backup privado antes de eliminar físicamente la base y su cuenta.</small></span></label><label><input type="radio" name="database-lifecycle" checked={mode === 'purge'} disabled={destructiveUnavailable} onChange={() => setMode('purge')}/><span><strong>Purgar sin backup</strong><small>Irreversible. Solo para datos que confirmas que no necesitas conservar.</small></span></label></fieldset>{destructiveUnavailable && <Alert>Redis comparte el mismo espacio de datos de la instancia y no admite purga por credencial. Puedes archivar la credencial o retirar la instancia completa.</Alert>}{mode === 'backup-purge' && <Field label="Carpeta de backups (opcional)" help="Si queda vacía se usa la carpeta privada predeterminada de NearProd."><input value={directory} onChange={e => setDirectory(e.target.value)} placeholder="~/.nearprod/backups/databases"/></Field>}{mode === 'purge' && <><Alert error>No habrá backup ni rollback. Escribe exactamente <code>{db.id}</code> y confirma la pérdida de datos.</Alert><Field label="ID exacto de la base"><input value={typedId} onChange={e => setTypedId(e.target.value)} autoComplete="off"/></Field><label className="check-label"><input type="checkbox" checked={acknowledge} onChange={e => setAcknowledge(e.target.checked)}/>Entiendo que la base y su cuenta se eliminarán físicamente.</label></>}{error && <Alert error>{error}</Alert>}<div className="modal-actions"><button type="button" onClick={onClose}>Cancelar</button><button className={mode === 'archive' ? 'primary' : 'danger'} disabled={busy || !ready}>{busy ? 'Revisando…' : 'Revisar operación'}</button></div></form></Modal>;
}
