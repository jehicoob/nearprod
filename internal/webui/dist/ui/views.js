...
export function RuntimeView({ runtime, connected, operations, onSaved, notify }) {
    ...
    async function load() {
        ...
        await Promise.allSettled([
            api('/doctor', undefined, ctrl.signal).then(d => { if (!ctrl.signal.aborted) {
                setDoctor(d);
                if (d.allocation) {
                ...
                setError(message(e)); }).finally(() => { if (!ctrl.signal.aborted)
                setLoadingDoctor(false); }),
            api('/metrics', undefined, ctrl.signal).then(m => { if (!ctrl.signal.aborted)
                setMetrics(m); }).catch(e => { if (!ctrl.signal.aborted)
                setError(message(e)); }).finally(() => { if (!ctrl.signal.aborted)
    ...
    async function task(work) { setBusy(true); setError(''); try {
        await work();
    }
    catch (e) {
        setError(message(e));
    ...
    return React.createElement("div", { className: "runtime-view" },
        ...
        React.createElement("div", { className: "runtime-grid" },
            React.createElement("section", { className: "panel" },
                ...
                React.createElement("form", { onSubmit: e => { e.preventDefault(); void task(async () => { await api('/runtime/settings', settings); onSaved(); notify('Contexto guardado. No se cambió el contexto global de Docker.'); }); } },
                    React.createElement(Field, { label: "Tipo de runtime", help: "En Mac, Colima ejecuta Docker en Linux. Docker local corresponde a Linux/WSL2; no elimina la necesidad de VM en macOS." },
                        React.createElement("select", { disabled: runtimeBusy, value: settings.kind, onChange: e => setSettings({ ...settings, kind: e.target.value }) },
                            React.createElement("option", { value: "colima" }, "Colima (macOS)"),
                            React.createElement("option", { value: "native" }, "Docker local (Linux / WSL2, experimental)"))),
            ...
            React.createElement("section", { className: "panel" },
                React.createElement("div", { className: "panel-heading" },
                    ...
                    React.createElement("h2", null, "M\u00E1quina virtual de Colima")),
                runtime.kind !== 'colima' ? React.createElement(Alert, null, "Docker utiliza el kernel de Linux/WSL2. Sus recursos se administran desde el sistema operativo; NearProd no ejecuta sudo.") : !doctor && loadingDoctor ? React.createElement(Skeleton, { label: "Consultando estado y asignaci\u00F3n de Colima", rows: 6 }) : React.createElement(React.Fragment, null,
                    ...
                    React.createElement("div", { className: "metric-pair" },
                        React.createElement("div", null,
                            ...
                            React.createElement("strong", null, doctor?.allocation ? `${doctor.allocation.memoryGiB} GiB` : 'Sin datos')),
                        React.createElement("div", null,
                    ...
                    React.createElement("div", { className: "inline-actions" },
                        React.createElement("button", { disabled: cannotChange || !doctor?.allocation || !['running', 'stopped'].includes(colimaState || ''), onClick: () => void task(async () => setPreview(await api('/runtime/preview', { memory: Number(memory), cpus: Number(cpus) }))) }, "Revisar cambio y alcance"),
                        colimaState === 'stopped' && !connected && React.createElement("button", { className: "primary", disabled: cannotChange, onClick: () => { if (window.confirm('¿Iniciar el perfil Colima existente? No se creará otra VM ni se cambiará el contexto global.'))
                                void task(async () => { await api('/runtime/actions', { action: 'start', confirm: true }); notify('Inicio de Colima solicitado. Consulta Actividad.'); }); } },
            ...
            !metrics && loadingMetrics ? React.createElement(Skeleton, { label: "Cargando consumo de recursos", rows: 4 }) : metrics ? React.createElement(React.Fragment, null,
                React.createElement("div", { className: "metrics" },
                    ...
                    React.createElement("div", null,
                        ...
                        React.createElement("strong", null, metrics.guest ? humanBytes(metrics.guest.swapTotalBytes - metrics.guest.swapFreeBytes) : 'Sin datos'))),
                React.createElement("p", { className: "hint" },
                    metrics.note,
                    " La pesta\u00F1a del navegador no est\u00E1 incluida en el RSS del agente. No sumes VM y contenedores como consumos independientes."),
                metrics.containers.length ? React.createElement("div", { className: "table-wrap" },
                    React.createElement("table", null,
                        ...
                        React.createElement("tbody", null, metrics.containers.map(c => React.createElement("tr", { key: c.id },
                            ...
                            React.createElement("td", null, c.memory)))))) : React.createElement("p", { className: "hint" }, connected ? 'No hay métricas de contenedores activos.' : 'Docker no está conectado. Las métricas de contenedores no están disponibles.')) : React.createElement(Alert, null, "No fue posible cargar las m\u00E9tricas. Vuelve a consultar el diagn\u00F3stico.")),
        React.createElement("section", { className: "panel" },
            React.createElement("div", { className: "panel-heading" },
                React.createElement(Icon, { name: "terminal" }),
                React.createElement("h2", null, "Herramientas y versiones")),
            React.createElement("p", { className: "field-help" }, "Son herramientas distintas, no VMs adicionales. Docker puede responder aunque falte un plugin del cliente."),
            !doctor && loadingDoctor ? React.createElement(Skeleton, { label: "Comprobando herramientas de Docker", rows: 4 }) : doctor && React.createElement(React.Fragment, null,
                React.createElement("p", { className: "hint" },
                    ...
                    " \u00B7 ",
                    doctor.platform),
                React.createElement("div", { className: "tool-list" }, doctor.tools.map(t => React.createElement("div", { key: t.name, className: "tool" },
                    React.createElement(Icon, { name: t.available ? 'check' : 'warning' }),
                    React.createElement("strong", null, t.name),
                    React.createElement("span", { className: "tool-status" }, t.available ? 'Disponible' : t.status === 'plugin-not-registered' ? 'Instalado, pero Docker no encuentra el plugin' : 'No disponible para NearProd'),
                    React.createElement("code", null, t.version),
                    React.createElement("p", { className: "field-help" },
                        t.purpose,
                        " ",
                        React.createElement("strong", null, t.required ? 'Requerido para su función.' : 'No bloquea estado, logs ni detención.')),
                ...
                doctor.tools.some(t => !t.available && ['Compose', 'Buildx'].includes(t.name)) && React.createElement("details", { className: "advanced" },
            ...
            React.createElement("p", { className: "hint" }, "Consulta metadatos locales de Homebrew. No instala paquetes ni actualiza dependencias de proyectos."),
            updates && React.createElement("div", { className: "update-results" },
        ...
        preview && React.createElement(Modal, { title: "Confirmar cambio global de Colima", subtitle: "La operaci\u00F3n afecta a toda la VM, no solo a NearProd.", onClose: () => setPreview(null) },
            ...
            React.createElement("div", { className: "modal-actions" },
                ...
                React.createElement("button", { className: "danger", disabled: busy || runtimeBusy, onClick: () => void task(async () => { await api('/runtime/actions', { ...preview, action: 'configure', confirm: true }); setPreview(null); notify('Cambio global solicitado. Consulta el resultado en Actividad.'); }) }, "Aplicar cambio global"))));
}
