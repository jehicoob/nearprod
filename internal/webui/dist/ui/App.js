...
function App() {
    ...
    async function load() {
        ...
        try {
            ...
            if (e instanceof ApiError && e.code === 'AUTH_REQUIRED')
                ...
                setStatus(current => ({ ...current, connected: false, checkedAt: new Date().toISOString(), error: { code: 'STATUS_UNAVAILABLE', message: message(e) } }));
        }
    }
    useEffect(() => { void load(); }, []);
    useEffect(() => {
    ...
    if (auth === null && loadError)
        return React.createElement("div", { className: "login-page" },
            React.createElement("section", { className: "login-card" },
                React.createElement("h1", null, "No se pudo abrir NearProd"),
                React.createElement(Alert, { error: true }, loadError),
                React.createElement("button", { className: "primary full", onClick: () => void load() }, "Reintentar conexi\u00F3n local")));
    if (auth === null)
        return React.createElement("div", { className: "login-page" },
    ...
    if (!auth)
        return React.createElement("div", { className: "login-page" },
            ...
            React.createElement("p", { className: "hint" }, "El panel funciona aunque Docker est\u00E9 detenido. No inicia tus contenedores autom\u00E1ticamente."));
    if (!catalog)
    ...
    return (React.createElement("div", { className: "shell" },
            ...
            React.createElement("div", { className: "sidebar-bottom" },
                React.createElement("div", { className: "host-card" },
                    ...
                    React.createElement("div", null,
                        ...
                        React.createElement("p", null, catalog.host.runtime.managedVirtualMachine
                            ? "Fuera de la VM"
        ...
        React.createElement("div", { className: "main" },
            React.createElement("header", { className: "topbar" },
                React.createElement("div", { className: "breadcrumb" },
                    ...
                    React.createElement("strong", null, page === "projects"
                        ? "Aplicaciones"
                        : page === "runtime"
                            ? "Runtime"
                            : page === "proxy"
            ...
            React.createElement("main", null,
                ...
                loadError && React.createElement(Alert, { error: true },
                    loadError,
                    " ",
                    React.createElement("button", { onClick: () => void load() }, "Reintentar")),
                    ...
                    React.createElement("div", { className: "overview" },
                        ...
                        React.createElement("div", null,
                            ...
                            React.createElement("strong", null,
                                ...
                                React.createElement("small", null, status.connected
                                    ? "incluye detenidos"
                    ...
                    groups.map((product) => (React.createElement("section", { className: "product", key: product },
                        React.createElement("div", { className: "product-heading" },
                            ...
                            React.createElement("div", { className: "product-actions" },
                                ...
                                React.createElement("button", { disabled: working ||
                                        !status.connected ||
                                        active.length > 0 ||
                                        !catalog.stacks.some((s) => s.product === product), onClick: () => void groupAction(product, "up") }, "Iniciar grupo"),
                                React.createElement("button", { disabled: working ||
                                        !status.connected ||
                                        active.length > 0 ||
                        ...
                        React.createElement("div", { className: "stacks" },
                            ...
                            filtered
                                ...
                                const isBusy = working ||
                                    active.some((o) => !o.targets.length || o.targets.includes(s.id));
                                return (React.createElement("article", { className: "stack", key: s.id },
                                    React.createElement("div", { className: "stack-main" },
                                        ...
                                        React.createElement("div", { className: "stack-mode" },
                                            React.createElement("label", null,
                                                "Modo",
                                                React.createElement("select", { "aria-label": `Modo ${s.id}`, value: selectedMode, onChange: (e) => setMode({
                                                        ...mode,
                                                        [s.id]: e.target.value,
                                                    }) },
                                                    React.createElement("option", { value: "dev" }, "Desarrollo"),
                                                    s.modes.verify && (React.createElement("option", { value: "verify" }, "Prueba de imagen")))),
                                            React.createElement("span", { className: "hint" }, s.activeMode
                                    ...
                                    (observed?.containers.length || 0) > 0 && (React.createElement("details", { className: "container-details" },
                                        ...
                                        React.createElement("div", { className: "container-list" }, observed?.containers.map((c) => (React.createElement("div", { className: "container", key: c.id },
                                            ...
                                            React.createElement("div", { className: "container-links" },
                                                c.ports
                                                    .filter((p, i, a) => a.findIndex((v) => v.port === p.port &&
                                                    v.container === p.container &&
                                                    v.host === p.host) === i)
                                                    .map((p) => p.url ? (React.createElement("a", { title: `Puerto publicado ${p.container}; protocolo web sugerido, sin comprobar conectividad`, key: `${p.container}-${p.host}-${p.port}`, href: p.url, target: "_blank", rel: "noreferrer noopener" },
                                                    ":",
                                                    p.port,
                                    ...
                                    React.createElement("div", { className: "watch-bar" },
                                        ...
                                        React.createElement("button", { disabled: !status.connected ||
                                                isBusy ||
                                                s.activeMode !== "dev", onClick: () => void work(async () => {
                                                await api("/watch", {
                                                    target: s.id,
                                                    action: "start",
                                                });
                                                notify("Watch solicitado; requiere develop.watch y un Compose compatible.");
                                            }) }, "Activar Watch"),
                                        observed?.watch.state !== "off" &&
                                            observed?.watch.state && (React.createElement(React.Fragment, null,
                                            React.createElement("span", null, observed.watch.state),
                                            React.createElement("button", { onClick: () => void work(async () => {
                                                    await api("/watch", {
                                                        target: s.id,
                                                        action: "stop",
                                                    });
        ...
        batch && (React.createElement(BatchEditor, { candidates: batch, groups: catalog.groups, onClose: () => setBatch(null), onSaved: () => {
                setBatch(null);
                void load();
                notify("Aplicaciones registradas en el grupo. No se ejecutó Docker.");
            } })),
        groupEditor && (React.createElement(GroupDialog, { ...groupEditor, onClose: () => setGroupEditor(null), onSaved: () => {
                setGroupEditor(null);
                void load();
        ...
        review && (React.createElement(ReviewDialog, { ...review, onClose: () => setReview(null), onApproved: () => {
                setReview(null);
                void load();
                notify("Configuración aprobada. Ahora puedes iniciarla explícitamente.");
            } })),
        ...
        adoption && (React.createElement(Modal, { title: "Revisar adopci\u00F3n", subtitle: adoption.id, onClose: () => setAdoption(null) },
            ...
            React.createElement("div", { className: "modal-actions" },
                ...
                React.createElement("button", { className: "primary", disabled: !adoption.allowed || working, onClick: () => void work(async () => {
                        await api("/adopt", {
                            target: adoption.id,
